...
if (( 5 <= $h )) && (( $h <= 9 )); then
    ...
    if [ "$(ping -q -w 1 -c 1 `ip r | grep default | cut -d ' ' -f 3`)" ]; then
        curl wttr.in/$location > $WEATHERFILE && notify-send "METEO @$location -- $(cat $WEATHERFILE | sed '10q;d' | grep -o '[A-Za-z]*' | awk '/[A-Za-z]*/{i++}i==1' | tr '[:upper:]' '[:lower:]'):" "$(cat $WEATHERFILE | sed '12q;d' | grep -Eo '[^m ]* [A-Za-z ]*' | grep '[A-Z]' | awk '{print $1 " " $2}' | awk '/[a-z]/{i++}i==1')  ☀ $(cat $WEATHERFILE | sed '13q;d' | grep -o 'm[0-9]*' | grep -o '[0-9]*' | awk '/[0-9]*/{i++}i==1')°C  ☂ $(cat $WEATHERFILE | sed '16q;d' | grep -o '[0-9\.]* mm . [0-9]*%' | awk '/[0-9\.]* mm/{i++}i==1')" -i $icon
    else
    ...
    if [ "$(ping -q -w 1 -c 1 `ip r | grep default | cut -d ' ' -f 3`)" ]; then
        curl wttr.in/$location > $WEATHERFILE && notify-send "METEO @$location -- $(cat $WEATHERFILE | sed '10q;d' | grep -o '[A-Za-z]*' | awk '/[A-Za-z]*/{i++}i==2' | tr '[:upper:]' '[:lower:]'):" "$(cat $WEATHERFILE | sed '12q;d' | grep -Eo '[^m ]* [A-Za-z ]*' | grep '[A-Z]' | awk '{print $1 " " $2}' | awk '/[a-z]/{i++}i==2')  ☀ $(cat $WEATHERFILE | sed '13q;d' | grep -o 'm[0-9]*' | grep -o '[0-9]*' | awk '/[0-9]*/{i++}i==2')°C  ☂ $(cat $WEATHERFILE | sed '16q;d' | grep -o '[0-9\.]* mm . [0-9]*%' | awk '/[0-9\.]* mm/{i++}i==2')" -i $icon
    else
        notify-send "METEO" "Check internet connection!" -i $icon;
    fi
