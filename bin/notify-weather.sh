...
# Last update: 15.05.18, 13:30:54 @x200

# Doc: Send notify about wather

location=Falenica
icon=/home/piotr/.dotfiles/i3/bin/weather-icon.png

if [ "$(ping -q -w 1 -c 1 `ip r | grep default | cut -d ' ' -f 3`)" ]; then
    curl wttr.in/$location > /home/piotr/.weatherreport && notify-send "METEO @$location" "$(cat ~/.weatherreport | sed '3q;d' | sed 's/.*0m //' | sed 's/[ \t]*//') ☀️ $(cat ~/.weatherreport | sed '4q;d' | grep -o 'm[0-9]*' | grep -o '[0-9]*' | tr '\n' '-' | sed -r 's/(.*)-/\1/')°C ☂ $(cat ~/.weatherreport | sed '7q;d' | grep -o '[0-9\.]* mm' | sed 's/0m //')" -i $icon;
else
