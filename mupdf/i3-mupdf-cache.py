...
# Last update: 2019-06-03, 13:20:36 @toshiba
# ** DOC
...
def mupdf_cache():
    ...
    tpd_re = re.compile(
        r"([\[\]\(\)a-z0-9-_.ąćęłńóśźż ]*.pdf) - ([0-9]*)/[0-9]* \(([0-9]*) dpi\)", re.I
    )
    ...
    index_n, keyN = r.select(
        "do you want to name the cached file? [y/n]", ["yes", "no"]
    )
    if index_n == 0:
        name_file = r.text_entry("enter the name:")
        cache_file = dt.now().strftime(
            f"%y%m %d %b @%H:%M:%S {name_file} ({len(all_args_list)})"
        )
    elif keyN == -1:
        quit()
    else:
